...
async def test_script_engine_prompt_forces_selected_commentary_language():
    ...
    await engine.generate_viral_script("mma hooks", commentary_language="en")

    assert "commentary_text must be written in English" in llm.prompt
    assert "title must be written in English" in llm.prompt
